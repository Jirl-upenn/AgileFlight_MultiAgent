    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.memories:
            for uid in self.possible_agents:
                ...
                self.memories[uid].create_tensor(name="stuck", size=1, dtype=torch.bool)
    ...
    def record_transition(
        ...
        if self.memories:
            ...
            for uid in self.possible_agents:
                ...
                # get stuck status from infos if available, otherwise default to False
                stuck = infos.get(uid, {}).get("stuck", torch.zeros_like(terminated[uid], dtype=torch.bool))
                # Ensure stuck has same shape as terminated (num_envs, 1)
                if stuck.shape != terminated[uid].shape:
                    stuck = stuck.view_as(terminated[uid])
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        def compute_gae(
            ...
            for i in reversed(range(memory_size)):
                ...
                # Set advantage to 0 for stuck drones (element-wise)
                if stuck is not None:
                    # stuck[i] is shape (num_envs, 1), so we mask advantage where stuck is True
                    advantage = advantage * (~stuck[i]).float()
        ...
        for uid in self.possible_agents:
            ...
            returns, advantages = compute_gae(
                ...
                stuck=memory.get_tensor_by_name("stuck"),
