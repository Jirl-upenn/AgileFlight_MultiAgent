...
class QuadcopterEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: QuadcopterEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._default_timeout_trigger_step = max(int(self.max_episode_length) - 1, 0)
        self._timeout_trigger_step_buf = torch.full(
            (self.num_envs,),
            fill_value=self._default_timeout_trigger_step,
            dtype=torch.long,
            device=self.device,
        )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # timeout conditions
        time_out = self.episode_length_buf >= self._timeout_trigger_step_buf
        if not self.cfg.is_train:
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        n_reset = len(env_ids)
        self._timeout_trigger_step_buf[env_ids] = self._default_timeout_trigger_step
        if self.cfg.is_train and self.cfg.stagger_full_resets and n_reset == self.num_envs and self.num_envs > 1:
            # Spread out only the timeout phases. Keep the true episode clock fresh
            # so non-timeout logic continues to see real post-reset elapsed time.
            self._timeout_trigger_step_buf[env_ids] = torch.randint_like(
                self._timeout_trigger_step_buf[env_ids],
        ...
        self._prev_x_drone_wrt_gate[env_ids] = 1.0
